...
const MlDisableMenuRouter = {
    ...
    mounted(){
        ...
        $this.item && $this.item.meta.dialog && $div.addEventListener('click', (e) => {
            e.stopPropagation();
    ...
    methods: {
        async showDialog(item: any){
            ...
            let result;
            if(item.component.mixin){
                result = item.component
            }else{
                const _r = await item.component()
                result = _r.default
            }
            const $component = new dialogVue()
            $component.$mount($div);
            $component.cropperShow = true;
            $component.open(new result(), item.meta.title)
        }
    }
}
const MlMenu = {
